...
subject to EV_SOC_Init{w in W}:
  SOC_EV[w,1] = if EV_arrival[w] == 1 then SOC_EV_initial[w] else 0;

# 修正后的EV SOC限制约束
subject to EV_SOC_Limits{w in W, t in T: t >= EV_arrival[w] and t <= EV_departure[w]}:
  SOC_EV_min[w] <= SOC_EV[w,t] <= SOC_EV_max[w];

subject to EV_SOC_Zero{w in W, t in T: t < EV_arrival[w] or t > EV_departure[w]}:
  SOC_EV[w,t] = 0;
...
subject to EV_Charge_Limit{w in W, t in T: t >= EV_arrival[w] and t <= EV_departure[w]}:
  EV_charge[w,t] <= EV_charge_rate[w] * u_EV_charge[w,t];

subject to EV_Discharge_Limit{w in W, t in T: t >= EV_arrival[w] and t <= EV_departure[w]}:
  EV_discharge[w,t] <= EV_discharge_rate[w] * u_EV_discharge[w,t];

subject to EV_NoActivity{w in W, t in T: t < EV_arrival[w] or t > EV_departure[w]}:
  EV_charge[w,t] = 0 and EV_discharge[w,t] = 0;
