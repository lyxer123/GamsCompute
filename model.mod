...
# ================== 新增输出变量 ==================
var Total_DW{W,T} >= 0;      # 总洗碗机功率
var Total_WM{W,T} >= 0;      # 总洗衣机功率
var Total_HVAC{W,T} >= 0;    # 总空调功率
var Total_InfLoad{W,T} >= 0; # 总不可调负载
var Total_ESS_charge{W,T} >= 0; # 总ESS充电功率
var Total_EV_charge{W,T} >= 0;  # 总EV充电功率
var Total_PV{W,T} >= 0;      # 总光伏功率
var Total_ESS_discharge{W,T} >= 0; # 总ESS放电功率
var Total_EV_discharge{W,T} >= 0;  # 总EV放电功率
var Total_Grid{W,T} >= 0;    # 总电网输入功率   
...
subject to Calculate_WM{w in W, t in T}:
    Total_WM[w,t] = sum{f in F} (x_phase_active[w,t,'WM',f] * P_consumption[w,'WM',f]);

subject to Calculate_HVAC{w in W, t in T}:
    Total_HVAC[w,t] = sum{f in F} (x_phase_active[w,t,'HVAC',f] * P_consumption[w,'HVAC',f]);
